...
const CustomYAxisTick = ({ x, y, payload, paymentMethods }: any) => {
  ...
  if (lowerName.includes('cod')) {
    return (
      <g transform={`translate(${x - 54}, ${y - 10})`}>
        <foreignObject width="48" height="20">
          <img src="/cod-icon.webp" alt="COD" className="h-5 w-12 object-contain" />
        </foreignObject>
      </g>
    );
  }
                      ...
                      {isLoadingContent ? (
                        ...
                        <ChartContainer config={paymentMethodsChartConfig} className="w-full h-full">
                          <RechartsBarChart data={paymentMethodData} layout="vertical" margin={{ top: 5, right: 60, left: 10, bottom: 5 }}>
                            <YAxis dataKey="name" type="category" tick={(props) => <CustomYAxisTick {...props} paymentMethods={allPaymentMethods} />} width={56} stroke="hsl(var(--border))" axisLine={false} tickLine={false} />
