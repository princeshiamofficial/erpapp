...
interface CreateOrderDialogFormData {
  ...
  address: string;
  phoneNumber: string;
  orderItems: Array<{ // This is the DialogOrderItem structure from client
    id: string; 
    model: string;
    quantity: string; // Comes as string from input
    lamination: string;
    unitPrice: number | null; // Set by model selection
    lineItemTotalPrice: number | null; // Calculated
  }>;
  advancePayment?: string | null; // Can be empty string
  ...
  try {
    ...
    for (const item of data.orderItems) {
      ...
      processedOrderItems.push({
        id: item.id, // Use the ID generated by the client (or existing ID if editing)
...
export async function updateOrderAction(
  orderId: string,
  updates: Partial<TrackingLink> // Changed to Partial<TrackingLink> to allow orderItems update
  ...
  try {
    ...
    // Basic validation for top-level updates
    ...
    // Validation for orderItems if present in updates
...
export async function assignDrToOrderAction(
  ...
  try {
    if (!actingUser || !actingUser.id || !actingUser.name) {
     ...
     if (readyForDesignStatusId !== 'ready-for-design') { // Hardcoded check for security
