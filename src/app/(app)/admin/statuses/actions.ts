...
export async function addStatusAction(name: string, color: string, isVisible: boolean): Promise<{ success: boolean; status?: CustomStatus; error?: string }> {
  try {
    const newStatus = await addStatus(name, color, isVisible);
    if (newStatus) {
      revalidatePath("/(app)/admin/statuses");
      revalidatePath("/(app)/orders"); // For status dropdowns
      revalidatePath("/(app)/tracking-links"); // For status dropdowns
...
export async function updateStatusAction(
  ...
  color: string, 
  isVisible: boolean,
  actingUserRole: UserRole // New parameter
): Promise<{ success: boolean; error?: string }> {
  try {
    const success = await updateStatus(id, name, color, isVisible, actingUserRole); // Pass actingUserRole
    if (success) {
      revalidatePath("/(app)/admin/statuses");
      revalidatePath("/(app)/orders");
      revalidatePath("/(app)/tracking-links");
      revalidatePath("/track/[trackingId]", "layout"); // Revalidate all tracking pages
      return { success: true };
    }
