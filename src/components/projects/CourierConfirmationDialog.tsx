...
export function CourierConfirmationDialog({ isOpen, onOpenChange, project, currentUser, onSuccess }: CourierConfirmationDialogProps) {
    ...
    if (isOpen && project) {
      ...
      const fetchOrderDetails = async () => {
        ...
        if (order) {
          ...
          setEditableAddress(order.address);
        } else {
  ...
  const handleConfirm = async () => {
    if (!project || !currentUser || !orderDetails) return;
    ...
    const result = await transferToCourierAction(
      project,
      currentUser,
      shippingArea,
      charge,
        ...
        {orderDetails && !isLoadingDetails && (
          <div className="text-sm text-foreground bg-secondary/50 p-4 rounded-md border border-border/50 space-y-3">
            ...
            <div className="grid grid-cols-3 items-center gap-4">
              ...
              <Input id="phone" value={orderDetails.phone} readOnly className="col-span-2 h-8 bg-muted/50 cursor-not-allowed" />
            </div>
