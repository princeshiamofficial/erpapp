...
interface GiftCourierDialogProps {
  isOpen: boolean;
...
export function GiftCourierDialog({ isOpen, onOpenChange, gift, currentUser, onSuccess }: GiftCourierDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: result.error || "Could not transfer gift to courier.",
        variant: "destructive",
        duration: 8000,
      });
    }
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            <Truck className="h-6 w-6 text-primary" /> Transfer Gift to Courier
          </AlertDialogTitle>
          <AlertDialogDescription>
            This will create a consignment in <span className="font-semibold text-foreground">SteadFast</span> for gift <span className="font-mono bg-muted px-1.5 py-0.5 rounded">{gift?.giftIdDisplay}</span>.
          </AlertDialogDescription>
        </AlertDialogHeader>
