...
export function QuotationDetailsClient({ quotation: initialQuotation, allStatuses, allUsers }: QuotationDetailsClientProps) {
  ...
  const orderSubtotal = Array.isArray(quotation.orderItems) ? quotation.orderItems.reduce((acc, item) => acc + (item.lineItemTotalPrice || 0), 0) : 0;
  const effectiveDiscount = quotation.specialClientDiscount || 0;
  ...
  const allAdvancePaymentRecords = useMemo(() => {
    ...
    if (quotation.advancePayments && quotation.advancePayments.length > 0) {
      records.push(...quotation.advancePayments);
    } else if (quotation.advancePayment && quotation.advancePayment > 0) {
      records.push({
        id: 'legacy-advance',
  ...
  const shippingCharge = quotation.shippingCharge || 0;
